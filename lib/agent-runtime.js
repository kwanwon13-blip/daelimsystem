...
function resolveStoredTemplateArg(skillSlug) {
  ...
  if (skillSlug === 'haatz-ledger' || skillSlug === 'nicetech-ledger') {
    const dir = getSkillTemplateDir(skillSlug);
    return dir || stored[0].path;
  }
  ...
  for (const a of files) {
    ...
    if (ExcelJS) {
      try {
        ...
        if (ws) {
          const a1 = ws.getCell('A1').value;
          fileSkill = fileSkill || vendorSlugFromText(String(a1 == null ? '' : (a1.text || a1)));
        }
      } catch (_) {}
    }
...
async function* runAgent({ userId, task, attachmentPaths = [], signal } = {}) {
  ...
  try {
    ...
    // 최종 파일 스캔
    const finalFiles = scanWorkspaceFiles(session.dir);
    const newFiles = finalFiles.filter(f => fileChangedSinceSnapshot(f, baseFileSnapshot));
