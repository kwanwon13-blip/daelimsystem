...
const PARTNER_LEDGER_VENDOR_RE = /한신공영|요진건설|홍지이앤씨|삼진비티|선두종합기술|익스테리어앤|한국지오텍|금광스틸/i;
...
function resolveStoredTemplateArg(skillSlug) {
  ...
  if (skillSlug === 'haatz-ledger' || skillSlug === 'nicetech-ledger' || skillSlug === 'partner-ledger' || skillSlug === 'posco-statement') {
    const dir = getSkillTemplateDir(skillSlug);
...
async function classifySkillXlsxFiles(attachments, skillSlug) {
  ...
  const vendorWords = {
    ...
    'partner-ledger': ['한신공영', '요진건설', '홍지이앤씨', '삼진비티', '선두종합기술', '익스테리어앤', '한국지오텍', '금광스틸'],
