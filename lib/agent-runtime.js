...
function detectLedgerSkillSlug(task, attachments = []) {
  const text = [
    String(task || ''),
...
function loadSkillContext(slug) {
  if (!slug) return [];
  const skillDir = path.join(APP_ROOT, '.claude', 'skills', slug);
  ...
  const lines = [
    ...
    '',
    `- 스킬: ${slug}`,
    '- 아래 절차를 우선 적용하고, 현재 첨부 파일만 원본 데이터로 사용하세요.',
    '- scripts/ 자원이 있으면 같은 로직을 새로 만들지 말고 해당 스크립트를 우선 실행하세요.',
    '',
  ...
  if (fs.existsSync(scriptsDir)) {
    ...
    if (scripts.length) {
      lines.push('', '### 번들 스크립트', ...scripts.map(script => `- ${script}`));
    }
  }
  lines.push('');
  return lines;
...
function buildSystemContext(session, attachments, task = '') {
  let lines = [
    '# Agent 작업 환경 안내',
    '',
  ...
  const skillSlug = detectLedgerSkillSlug(task, attachments);
...
async function* runAgent({ userId, task, attachmentPaths = [], signal } = {}) {
  ...
  try {
    ...
    const systemContext = buildSystemContext(session, attachments, task);
