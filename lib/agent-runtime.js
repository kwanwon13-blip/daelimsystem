...
async function classifySkillXlsxFiles(attachments, skillSlug) {
  let ExcelJS;
  ...
  for (const a of files) {
    ...
    const looksRaw = sheetNames.includes('판매현황') || /판매현황|raw|sales/.test(name);
    const looksTemplate = /마감내역서|거래명세|template|템플릿|전월/.test(name + ' ' + sheetText)
      || vendorWords.some(w => name.includes(w.toLowerCase())) && !looksRaw;
    if (looksRaw) raw.push(a);
    else if (looksTemplate) templates.push(a);
    else templates.push(a);
  }
  return { raw: raw[0] || null, template: templates[0] || null, files };
}

async function* runBundledScriptSkill({ skillSlug, session, attachments, baseFiles, signal }) {
  ...
  if (!classified.raw) {
    yield { type: 'error', data: { message: `${skillSlug} 스킬을 실행하려면 '판매현황' 시트가 있는 원본 xlsx 첨부가 필요합니다.` } };
    return;
  }
  ...
  if (classified.template) args.push('--template', classified.template.path);

  yield { type: 'output', data: { text: `스킬 실행: ${skillSlug}\n원본: ${classified.raw.name}\n${classified.template ? `템플릿: ${classified.template.name}\n` : '템플릿: 첨부/작업폴더에서 자동 탐색\n'}` } };
  ...
  const finalFiles = scanWorkspaceFiles(session.dir)
    .filter(f => !baseFiles.has(f.relPath));
...
async function* runAgent({ userId, task, attachmentPaths = [], signal } = {}) {
  ...
  try {
    ...
    const skillSlug = await detectLedgerSkillSlug(task, attachments);
    if (BUNDLED_SCRIPT_SKILLS[skillSlug]) {
      for await (const evt of runBundledScriptSkill({ skillSlug, session, attachments, baseFiles, signal })) {
        yield evt;
      }
      return;
    }
    ...
    const args = ['-p', '--permission-mode', PERMISSION_MODE, '--add-dir', APP_ROOT];
    ...
    // 최종 파일 스캔
    const finalFiles = scanWorkspaceFiles(session.dir);
    const newFiles = finalFiles.filter(f => !baseFiles.has(f.relPath));
