...
async function* runAgent({ userId, task, attachmentPaths = [], signal } = {}) {
  ...
  try {
    ...
    // ── 정직한 라우터: 모르는 거래처 마감 요청이면 일반 에이전트가 멋대로 만들지 못하게 멈춤 ──
    {
      const { unknownLedgerGuard } = require('./ledger-router');
      const guard = unknownLedgerGuard({ matchedSlugs: bundledSkillSlugs, task, attachments });
      if (guard.stop) {
        yield { type: 'output', data: { text: guard.message } };
        yield { type: 'done', data: {
          sessionId: session.sessionId,
          dir: session.dir,
          exitCode: 0,
