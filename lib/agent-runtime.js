...
// 기본 'print'(-p) — 안정적인 헤드리스 모드(챗과 동일 방식). interactive 는 stdin 파이프+done파일
// 대기라 서버에서 응답 없이 멈추는 사고가 있어 opt-in(AGENT_CLAUDE_DRIVER=interactive)으로만.
const CLAUDE_DRIVER = String(process.env.AGENT_CLAUDE_DRIVER || 'print').trim().toLowerCase();
...
// 첨부된 판매현황 xlsx 의 "내용"으로 거래처 감지 — 파일명이 아니라 시트 안을 본다.
// 이카운트 랜덤 export 는 파일명이 무의미하고 거래처명이 A1 에 없을 수 있어,
// 위쪽 헤더 + 거래처명 컬럼 구간(최대 30행 × 20열)을 넓게 훑어 거래처명을 찾는다.
async function detectVendorFromAttachments(attachments = []) {
  ...
  for (const a of attachments) {
    ...
    try {
      ...
      // '판매현황' 우선, 없으면 데이터 있는 첫 시트 (빈/수식전용 시트를 데이터로 오인하지 않게)
      const ws = wb.getWorksheet('판매현황') || wb.worksheets.find(w => (w.rowCount || 0) > 0) || wb.worksheets[0];
      if (!ws) continue;
      const pieces = [];
      const maxR = Math.min(ws.rowCount || 30, 30);
      const maxC = Math.min(ws.columnCount || 20, 20);
      for (let r = 1; r <= maxR && pieces.length <= 600; r++) {
        for (let c = 1; c <= maxC; c++) pieces.push(exceljsValueText(ws.getCell(r, c).value));
      }
      const slug = anyVendorSlugFromText(pieces.join(' '));
      if (slug) return slug;
    } catch (_) { continue; }
  }
  return '';
...
async function detectLedgerSkillSlug(task, attachments = []) {
  ...
  // 1) task/파일명에 회사명이 직접 있으면 그걸로 (등록 거래처 포함)
  const direct = anyVendorSlugFromText(text);
...
const { resolveScriptName: _resolveScriptName, listRegistryVendors: _listRegistryVendors } = require('./skill-registry');
const { LEDGER_INTENT } = require('./ledger-router'); // 마감 의도 SSOT — 라우터와 같은 정의를 쓴다
const SKILL_REGISTRY_PATH = path.join(APP_ROOT, 'data', 'ai-skill-registry.json');
// 번들(고정) + 동적(승인 등록) 스킬의 스크립트명 해석. 없으면 null. (번들 우선 → 기존 동작 불변)
function reusableScriptName(slug) {
  return _resolveScriptName(slug, { bundled: BUNDLED_SCRIPT_SKILLS, registryPath: SKILL_REGISTRY_PATH });
}
// 등록(승격)된 거래처명을 텍스트에서 찾아 슬러그 반환 — 하드코딩 거래처(vendorSlugFromText)의 동적 보완.
// 이게 없으면 사장님이 승인해 등록한 거래처도 파일 "내용"으로는 영영 감지되지 않는다.
function dynamicVendorSlugFromText(text, registryPath = SKILL_REGISTRY_PATH) {
  const t = String(text || '');
  if (!t) return '';
  try {
    // 긴 이름(더 구체적)부터 검사, 3글자 이상만 — 짧은 이름이 다른 단어에 부분일치하는 오탐 방지
    // (예: '대명'이 '대명건설'에 걸리는 일 차단). 2글자 거래처는 내용 자동감지에서 제외(명시 등록은 가능).
    const vendors = _listRegistryVendors(registryPath)
      .filter(v => v.name.length >= 3)
      .sort((a, b) => b.name.length - a.name.length);
    for (const v of vendors) {
      if (t.includes(v.name)) return v.slug;
    }
  } catch (_) {}
  return '';
}
function anyVendorSlugFromText(text) {
  return vendorSlugFromText(text) || dynamicVendorSlugFromText(text);
}
...
async function detectBundledSkillSlugs(task, attachments = []) {
  ...
  const textSkill = anyVendorSlugFromText(text);
  if (textSkill && reusableScriptName(textSkill)) found.add(textSkill);

  let ExcelJS;
  try { ExcelJS = require('exceljs'); } catch (_) { ExcelJS = null; }
  for (const a of attachments || []) {
    if (!a || !a.path || !/\.xlsx$/i.test(a.path) || !fs.existsSync(a.path)) continue;
    let skill = anyVendorSlugFromText(`${a.original || ''} ${a.name || ''}`);
    if (!skill && ExcelJS) {
      try {
        const wb = new ExcelJS.Workbook();
        await wb.xlsx.readFile(a.path);
        ...
        if (ws) {
          const a1 = ws.getCell('A1').value;
          skill = anyVendorSlugFromText(String(a1 == null ? '' : (a1.text || a1)));
        }
      } catch (_) {}
    }
    if (skill && reusableScriptName(skill)) found.add(skill);
  }

  if (!found.size) {
    const single = await detectLedgerSkillSlug(task, attachments);
    if (single && reusableScriptName(single)) found.add(single);
  }
  return [...found];
}

async function* runBundledScriptSkill({ skillSlug, session, attachments, baseFiles, signal, strictTemplates = false }) {
  const scriptName = reusableScriptName(skillSlug);
...
async function* runAgent({ userId, task, attachmentPaths = [], signal } = {}) {
  ...
  try {
    ...
    // ── 정직한 라우터 + 처음 작업 분류 (skill / passthrough / ask-template / generate) ──
    let generationPrefix = '';
    {
      const { classifyUnknownLedger, buildGenerationInstruction } = require('./ledger-router');
      const cls = classifyUnknownLedger({ matchedSlugs: bundledSkillSlugs, task, attachments });
      if (cls.mode === 'ask-template') {
        // 모르는 거래처인데 양식 없음 → 멋대로 만들지 말고 정직하게 멈춤
        yield { type: 'output', data: { text: cls.message } };
        yield { type: 'done', data: {
          sessionId: session.sessionId,
          dir: session.dir,
          exitCode: 0,
      ...
      if (cls.mode === 'generate') {
        // 원본+양식 둘 다 있음 → 양식 그대로 틀을 만들도록 지시(결과는 임시)
        generationPrefix = buildGenerationInstruction({ task }) + '\n\n';
        yield { type: 'output', data: { text: '🆕 처음 하는 거래처예요. 첨부한 전월 양식 그대로 처리해볼게요.\n(결과는 "임시" — 사장님 확인·등록 후 정식 사용됩니다)\n' } };
      }
    }
    ...
    const fullPrompt = systemContext + '\n\n' + generationPrefix + task.trim() + doneInstruction;

    // claude CLI driver. 기본은 print(-p) 헤드리스 — 챗과 동일하게 안정적.
    // interactive 모드는 stdin 파이프+done파일 대기 방식이라 서버에서 멈추는 사고가 있어 opt-in.
    const useInteractiveDriver = CLAUDE_DRIVER === 'interactive';
    // ⚠ 보안(2026-06-13): 이전엔 --add-dir APP_ROOT 로 에이전트에게 앱 전체(.env, data/*.db 등)
    // 접근을 허용했다. 스킬 디렉터리로만 한정해 비밀/PII 노출면을 줄인다. (작업은 워크스페이스 cwd 안에서)
    const AGENT_ADD_DIR = path.join(APP_ROOT, '.claude', 'skills');
    const args = useInteractiveDriver
      ? ['--model', CLAUDE_MODEL, '--dangerously-skip-permissions', '--add-dir', AGENT_ADD_DIR]
      : ['-p', '--model', CLAUDE_MODEL, '--permission-mode', PERMISSION_MODE, '--add-dir', AGENT_ADD_DIR];
    ...
    // ── 자동점검은 "매번": 처음 작업(generate)으로 만든 산출물도 점검 (스펙 §4.5).
    //    파일 0개(만들다 만 경우)도 🚩로 잡힌다. 점검 실패가 결과 반환을 막지는 않는다.
    let generalCheck = null;
    if (generationPrefix) {
      try {
        const { autocheckLedger } = require('./ledger-autocheck');
        const verdict = await autocheckLedger({ files: newFiles, dir: session.dir, stdout: '' });
        yield { type: 'output', data: { text: '\n' + verdict.text + '\n' } };
        generalCheck = { status: verdict.status, reasons: verdict.reasons, summary: verdict.summary };
      } catch (_) { /* 점검 실패는 무시 */ }
    }
...
module.exports = {
  ...
  getStats,
  resolveSessionFile,
  // 거래처 자동감지 (파일 내용 기반 — 테스트/외부 호출용)
  detectVendorFromAttachments,
  vendorSlugFromText,
  dynamicVendorSlugFromText,
  anyVendorSlugFromText,
