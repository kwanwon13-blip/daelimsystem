...
function _spawnClaudeCli(prompt, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    // 라이브 서버 claude CLI 가 모든 호출에서 exit 1 로 죽던 문제의 핵심 = MCP 커넥터.
    //   서버엔 claude.ai 커넥터(MCP)가 pending/needs-auth 상태라, --strict-mcp-config 없이 spawn 하면
    //   CLI 가 그걸 로딩하다 exit 1. 동작하는 스트리밍 챗(lib/claude-cli.js)은 --strict-mcp-config 를 줘서 회피함.
    //   여기서도 동일하게: --strict-mcp-config(MCP 미로딩) + --model(서버 기본값 무시) + bypassPermissions(헤드리스).
    const appRoot = path.join(__dirname, '..');
    const cliArgs = ['-p', '--model', DEFAULT_MODEL, '--permission-mode', 'bypassPermissions', '--strict-mcp-config', '--add-dir', appRoot];
