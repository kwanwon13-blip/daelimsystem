...
/** 품목 라인 1줄 파싱 → { itemName, spec, qty, unit, remark? } (순수, best-effort)
 *  괄호 (xxx) 는 비고(remark)로 보존하고 품목명에서 분리한다. */
function parseItemLine(line) {
  let s = String(line).replace(/^[-*•]\s*/, '').trim();
  // 괄호 (...) → 비고로 추출 (여러 개면 ' / '로 이어붙임)
  let remark = '';
  s = s.replace(/[（(]([^（）()]*)[）)]/g, (_, inner) => {
    const t = String(inner).trim();
    if (t) remark = remark ? remark + ' / ' + t : t;
    return ' ';
  }).replace(/\s+/g, ' ').trim();
  let qty = 0, unit = '';
...
/** 카톡 글 → [{ vendorGuess, items:[{itemName,spec,qty,unit,remark?}] }] (순수, 저장 전 후보).
...
/** 업체명 정규화 (소문자·공백제거) — 자유 업체명 그룹핑/매칭 공용 */
...
function groupByVendor(requests) {
  ...
  for (const r of (requests || [])) {
    ...
    if (!map.has(key)) map.set(key, { vendorId: r.vendorId || null, vendorName: r.vendorName, requests: [], items: [] });
    const g = map.get(key);
    g.requests.push(r);
