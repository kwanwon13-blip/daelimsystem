...
/** 픽업 '추가요청' 알림 수신자 = '영업지원팀' 부서원(승인됨)의 userId 목록 (순수).
 *  - 부서명에 '영업지원'이 포함된 부서를 찾아 그 소속 승인 사용자만.
 *  - 해당 부서/사용자가 없으면 폴백으로 admin(승인됨) — 알림 유실 방지.
 *  매 등록이 아니라 마감(기본 13:00) 이후 추가요청에서만 호출된다(라우트 측 isLate 게이트). */
function salesSupportUserIds(org) {
  const depts = (org && org.departments) || [];
  const users = (org && org.users) || [];
  const norm = s => String(s || '').replace(/\s/g, '');
  const dept = depts.find(d => norm(d && d.name).includes('영업지원'));
  const approved = users.filter(u => u && u.status === 'approved');
  let ids = dept ? approved.filter(u => u.department === dept.id).map(u => u.userId || u.id) : [];
  if (!ids.length) ids = approved.filter(u => u.role === 'admin').map(u => u.userId || u.id);
...
module.exports = { computeRequestStatus, computeIsLate, parseItemLine, parseKakaoPickup, buildShareText, groupByVendor, normVendorName, looksLikeVendorHeader, salesSupportUserIds };
