...
function addMemory({ content, category, scope = 'company', createdBy = '', sourceKind = 'auto', originRole = 'user', sourceThreadId = null, sourceMessageId = null }) {
  ...
  try {
    ...
    // 3.5) 깨끗한(active) 사실만 퍼지 중복 차단 — norm_key 정확일치는 위(1)에서 거름.
    //      한국어 재서술("부가세 별도"↔"부가세별도임")은 음절 bigram 유사도로 흡수하되,
    //      숫자(단가·수량)가 다르면 다른 사실이라 병합 금지. pending/교정은 검토 경로 유지(여기 안 탐).
    if (status === 'active') {
      try {
        const dig = _digits(text);
        const near = db.prepare("SELECT id, content FROM chat_memory WHERE scope=? AND status='active'").all(scope);
        const hit = near.find(n => _digits(n.content) === dig && _bigramSim(n.content, text) >= 0.6);
        if (hit) {
          db.prepare('UPDATE chat_memory SET hit_count=hit_count+1, updated_at=?, source_thread_id=COALESCE(?,source_thread_id) WHERE id=?').run(now(), sourceThreadId, hit.id);
          return { id: hit.id, deduped: true, fuzzy: true };
        }
      } catch (_) { /* 퍼지 실패는 무해 — 신규 저장으로 진행 */ }
    }
...
// 숫자 시그니처(단가·수량 등) — 다르면 다른 사실이므로 퍼지 병합을 막는 가드에 사용
function _digits(s) { return (String(s || '').match(/\d+/g) || []).join(','); }
// 음절 bigram 집합(정규화 후) — 한국어 재서술 유사도 판정용
function _bigrams(s) { const n = filters.normKey(s); const g = new Set(); for (let i = 0; i < n.length - 1; i++) g.add(n.slice(i, i + 2)); return g; }
function _bigramSim(a, b) { const A = _bigrams(a), B = _bigrams(b); if (!A.size || !B.size) return 0; let i = 0; for (const x of A) if (B.has(x)) i++; return i / (A.size + B.size - i); }

// 관련도 랭킹: IDF 가중 키워드 겹침(흔한 단어↓·희귀 단어↑) + recency 5% 타이브레이커.
// 정확 문구 일치는 바닥 0.8(품목코드·거래처명 최우선). 비고정은 kw>0 일 때만(무관 컷).
// IDF의 df는 '실제 매칭 방식'(정확토큰 OR 정규화 부분문자열)으로 카운트 — 한글 조사로 토큰형이
// 달라도("단가는"↔"단가") 흔한 단어를 제대로 깎기 위함.
function _rank(rows, prompt) {
  ...
  const norm = rows.map(r => ({ toks: new Set(_tokens(r.content)), cnorm: filters.normKey(r.content) }));
  const dfMap = new Map();
  for (const t of new Set(ptoks)) {
    const nt = filters.normKey(t); let c = 0;
    for (const x of norm) if (x.toks.has(t) || x.cnorm.includes(nt)) c++;
    dfMap.set(t, c);
  }
  const idf = t => Math.log((N - (dfMap.get(t) || 0) + 0.5) / ((dfMap.get(t) || 0) + 0.5) + 1);
  const qIdf = ptoks.reduce((s, t) => s + idf(t), 0) || 1;
  const pnorm = filters.normKey(prompt);
  return rows.map((r, i) => {
    const x = norm[i];
    let w = 0;
    for (const t of ptoks) if (x.toks.has(t) || x.cnorm.includes(filters.normKey(t))) w += idf(t);
    let kw = w / qIdf;
    if (pnorm.length >= 2 && x.cnorm.includes(pnorm)) kw = Math.max(kw, 0.8);   // 정확 문구/코드 일치 최우선
...
// 승인 → active. 교정(부정) 발화면 같은 category 에서 같은 "주제어"를 공유하는 기존 active 를 supersede(archive).
function approveMemory(id) {
  ...
  try {
    ...
    if (filters.detectNegation(m.content).hit) {
      // 가장 긴 토큰(보통 거래처/품목명)을 주제어로. 끝 조사(은/는/이/가…) 영향 줄이려 1글자 뗀 변형도 후보.
      const longest = (m.content.match(/[가-힣]{2,}/g) || []).sort((x, y) => y.length - x.length)[0] || '';
      const stems = [];
      if (longest.length >= 2) {
        stems.push(filters.normKey(longest));
        if (longest.length >= 4) stems.push(filters.normKey(longest.slice(0, longest.length - 1)));
      }
      if (stems.length) {
        ...
        for (const c of cands) {
          const cn = filters.normKey(c.content);
          if (stems.some(s => s.length >= 3 && cn.includes(s))) {
