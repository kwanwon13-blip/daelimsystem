...
function matchOcrTextToPool(ocrText, opts = {}) {
  ...
  for (const p of products) {
    ...
    // 큰 차원 기준 (제일 긴 변) 으로 tolerance 산출 — 한쪽 변이 크면 다른쪽도 관대
    if (pool.length === 0 && pSpec.includes('*')) {
      const [pw, ph] = pSpec.split('*').map(Number);
      const maxDim = Math.max(pw, ph);
      // 큰 변 기준 tolerance: <1500 → 50mm / 1500-3000 → 3% / 3000+ → 5%
      let tol;
      if (maxDim < 1500) tol = 50;
      else if (maxDim < 3000) tol = Math.max(50, Math.round(maxDim * 0.03));
      else tol = Math.max(100, Math.round(maxDim * 0.05));
      pool = candidates.filter(r => {
        const rSpec = normalizeSpec(r.spec);
