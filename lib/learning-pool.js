...
function extractSlideMaterials(fullText) {
  ...
  // 후렉스 (다양한 표기 + 관련어 + 큰 사이즈 출력 키워드)
  // - 후렉스/세륜장간판/조회단상/현수나배/출력만/텐션 → 후렉스 계열
  if (/후렉스|후렉스간판|후렉스출력만|세륜장간판|조회단상|현수나배|아일렛후렉스|출력만|텐션용|텐션\s*바|상부.*아일렛|상부.*반생/.test(t)) mats.push('후렉스');
  // 포맥스
  ...
  // 스티커류
  if (/원형스티커|덧방스티커|투명스티커|배면스티커|반사스티커|유포실사스티커|실사스티커/.test(t)) mats.push('스티커');
  return [...new Set(mats)];
}
