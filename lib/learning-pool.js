...
function extractSlideMaterials(fullText) {
  ...
  // 스티커류
  if (/원형스티커|덧방스티커|투명스티커|배면스티커|반사스티커|유포실사스티커/.test(t)) mats.push('스티커');
  return [...new Set(mats)];
}
...
function matchOcrTextToPool(ocrText, opts = {}) {
  ...
  for (const p of products) {
    ...
    // 작은 사이즈 (1500↓): ±50mm 절대치
    // 중간 사이즈 (1500~3000): ±3%
    // 큰 사이즈 (3000+): ±5% (시안↔실제 매출 사이즈 자주 다름)
    if (pool.length === 0 && pSpec.includes('*')) {
      const [pw, ph] = pSpec.split('*').map(Number);
      function tolerance(dim) {
        if (dim < 1500) return 50;
        if (dim < 3000) return Math.max(50, dim * 0.03);
        return Math.max(100, dim * 0.05);
      }
      pool = candidates.filter(r => {
        const rSpec = normalizeSpec(r.spec);
        ...
        const tolW = tolerance(pw);
        const tolH = tolerance(ph);
        if (Math.abs(rw - pw) <= tolW && Math.abs(rh - ph) <= tolH) return true;
        // 회전된 경우도
