...
function matchSlideToProductMaster(slideText, opts = {}) {
  ...
  if (PM.rows === 0) {
    return { status: 'unknown', reason: '품목 마스터 미로드' };
  }

  // 1) 품목명 후보 추출 (정규화)
  // OCR 텍스트에서 본품 후보 토큰 뽑기 — 학습된 모든 상품명 중 가장 잘 맞는 거
  let bestNameMatch = null;
  let bestScore = 0;
  const tnorm = norm(text);
  for (const [nameKey, rows] of PM.byNameOnly) {
    if (!nameKey || nameKey.length < 2) continue;
    if (tnorm.includes(nameKey)) {
      // 더 긴 매치를 우선 (구체적인 상품명이 더 정확)
      if (nameKey.length > bestScore) {
        bestScore = nameKey.length;
        bestNameMatch = { key: nameKey, rows };
      }
    }
  }
  ...
  if (!bestNameMatch) {
    return {
      status: 'unknown',
      reason: '시안에서 마스터 품목명 못 찾음',
      extractedSpec,
      options,
    };
  }

  // 4) 같은 품명 + 규격으로 정확 매칭 시도
  const targetSpec = normalizeSpec(extractedSpec);
  const matchedExact = bestNameMatch.rows.find(r => normalizeSpec(r.spec) === targetSpec);
  if (matchedExact) {
    return {
      status: 'matched',
      ...
      priceOut: matchedExact.priceOut,
      priceIn: matchedExact.priceIn,
      options,
      reason: `품목코드 매칭 (${matchedExact.code})`,
    };
  }

  // 5) 같은 품명 다른 규격 → 예측 (단가 추정)
  if (bestNameMatch.rows.length > 0) {
    // 같은 품명의 출고단가 평균
    const validPrices = bestNameMatch.rows.filter(r => r.priceOut > 0).map(r => r.priceOut);
    const avgPrice = validPrices.length > 0
      ? Math.round(validPrices.reduce((a, b) => a + b, 0) / validPrices.length)
      : 0;
    const sample = bestNameMatch.rows[0];
    return {
      status: 'predicted',
      code: null,
      item: sample.item,                  // 같은 품명 사용
      spec: extractedSpec || sample.spec, // 시안에서 추출한 규격 (없으면 샘플)
      priceOut: avgPrice,                 // 같은 품명 평균 단가
      priceIn: 0,
      options,
      reason: `같은 품명(${sample.item}) ${bestNameMatch.rows.length}건 평균 단가 사용. 정확한 코드는 신규 등록 필요.`,
      suggestion: {
  ...
  return { status: 'unknown', reason: '매칭 실패', extractedSpec, options };
}
