...
// ⚠ 보안(2026-06-13): model 은 요청 본문(req.body.model)에서 흘러들어올 수 있고,
// spawn(..., { shell:true }) 의 argv 로 들어간다. cmd.exe 메타문자(& | < > ^ " ( ) % ! 공백 등)가
// 섞이면 명령 주입(RCE)이 되므로, 셸 안전 문자(모델 ID 문법)만 허용하고 아니면 기본 모델로 폴백한다.
function safeModel(m) {
  if (typeof m === 'string' && m.length <= 64 && /^[A-Za-z0-9._:\-\[\]]+$/.test(m)) return m;
 ...
 */
// SIGTERM 으로 안 죽는 좀비 방지 — 3초 뒤 SIGKILL 강제 종료 (타임아웃·중단 공통).
...
function callClaudeCli(prompt, attachmentPaths = [], opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    const tmpDir = path.join(os.tmpdir(), 'claude-cli-' + process.pid + '-' + Date.now() + '-' + crypto.randomBytes(6).toString('hex'));
