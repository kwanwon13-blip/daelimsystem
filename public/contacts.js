...
function contactsApp() {
  return {
    ...
    // 정렬
    sortField: '',               // 'name' | 'position' | 'mobile' | 'phone' | 'email'
    ...
    toast2Show: false,
    // 현장 이동 모달 (단일 연락처)
    contactMoveModal: { open: false, contact: null, targetCompanyId: '', targetProjectId: '' },
    // 현장을 다른 업체로 이동 모달
    ...
    async loadTree() {
      // 스크롤 위치 저장 (loadTree 재렌더링 후 복원)
      var _scrollEl = document.querySelector('.main-scroll');
      var _savedScroll = _scrollEl ? _scrollEl.scrollTop : window.scrollY;
      ...
      // 현장: 한글 ㄱ-ㅎ 자동 정렬 (수동 order가 다를 경우 order 우선)
      ...
      // 연락처를 추가된 순서대로(오래된 것 위) 정렬 - 새로 추가된 연락처가 아래에 표시
      function sortContactsAsc(arr) {
        arr.sort(function(a, b) {
          var ia = a.order !== undefined ? a.order : 99999;
          var ib = b.order !== undefined ? b.order : 99999;
          if (ia !== ib) return ia - ib;
          // order가 같으면 id(문자열) 비교 - 오래된 게 먼저
          return (String(a.id || '')).localeCompare(String(b.id || ''));
        });
      }
      Object.keys(contactByProject).forEach(function(k) { sortContactsAsc(contactByProject[k]); });
      Object.keys(contactNoProject).forEach(function(k) { sortContactsAsc(contactNoProject[k]); });
      ...
      // 스크롤 복원 (Alpine 재렌더링 후 충분한 딜레이)
      setTimeout(function() {
        var el = document.querySelector('.main-scroll');
        if (el) el.scrollTop = _savedScroll; else window.scrollTo(0, _savedScroll);
      }, 30);
    ...
    async addContact(companyId, projectId) {
      ...
      // 스크롤 위치 저장 (loadTree 후 복원)
      var scrollEl = document.querySelector('.main-scroll');
      var savedScroll = scrollEl ? scrollEl.scrollTop : window.scrollY;
      await this.loadTree();
      this.expandedCompanies.add(companyId);
      ...
      this.$nextTick(function() {
        // 스크롤 위치 복원
        var el = document.querySelector('.main-scroll');
        if (el) el.scrollTop = savedScroll; else window.scrollTo(0, savedScroll);
        self.startInlineEdit(saved, 'name');
      });
    },
    ...
    onReorderDragOver(e, type, id) {
      if (!this.reorderDragId) return;
      // 현장을 회사 헤더로 드래그할 때도 허용
      if (type === 'company' && this.reorderType === 'project') {
        e.preventDefault();
        e.dataTransfer.dropEffect = 'move';
        this.reorderOverId = id;
        return;
      }
      if (this.reorderType !== type) return;
    ...
    async onReorderDrop(e, type, targetId) {
      ...
      var dragType = this.reorderType; // 드래그 소스 타입 저장
      this.reorderDragId = '';
      this.reorderOverId = '';
      this.reorderType = '';
      this.reorderParentId = '';
      ...
      // ── 현장을 다른 회사 헤더로 드롭 ──
      if (type === 'company' && dragType === 'project') {
        var fromComp = null, toComp = null, movedProj = null;
        this.tree.forEach(function(c) {
          c.projects.forEach(function(p) { if (p.id === dragId) { fromComp = c; movedProj = p; } });
          if (c.id === targetId) toComp = c;
        });
        if (!fromComp || !toComp || fromComp.id === toComp.id || !movedProj) return;

        // 즉시 UI 업데이트 (optimistic)
        fromComp.projects = fromComp.projects.filter(function(p) { return p.id !== dragId; });
        movedProj.companyId = toComp.id;
        toComp.projects.push(movedProj);
        this.tree = this.tree.slice();
        this.expandedCompanies.add(toComp.id);
        this.expandedCompanies = new Set(this.expandedCompanies);
        restoreScroll();
        this.showToast2('현장이 이동됐습니다');

        // 백그라운드 API 저장
        fetch('/api/contacts/projects/' + dragId, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ companyId: toComp.id })
        }).then(function() { self.loadTree(); });
        return;
      }
      ...
      if (type === 'company') {
        // ── 업체 순서 변경 — optimistic ──
        var fromIdx = this.tree.findIndex(function(c) { return c.id === dragId; });
        var toIdx = this.tree.findIndex(function(c) { return c.id === targetId; });
        if (fromIdx < 0 || toIdx < 0) return;
        var moved = this.tree.splice(fromIdx, 1)[0];
        this.tree.splice(toIdx, 0, moved);
        this.tree = this.tree.slice();
        restoreScroll();
        this.showToast2('순서 변경됨');

        // 백그라운드 API (병렬)
        var ids = this.tree.map(function(c) { return c.id; });
        Promise.all(ids.map(function(id, i) {
          return fetch('/api/contacts/companies/' + id, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        var fromComp = null, toComp = null;
        this.tree.forEach(function(c) {
          c.projects.forEach(function(p) {
            if (p.id === dragId) fromComp = c;
            if (p.id === targetId) toComp = c;
          });
        });
        if (!fromComp || !toComp) return;

        if (fromComp.id === toComp.id) {
          // ── 같은 업체 내 순서 변경 — optimistic ──
          var fromIdx = fromComp.projects.findIndex(function(p) { return p.id === dragId; });
          var toIdx = fromComp.projects.findIndex(function(p) { return p.id === targetId; });
          if (fromIdx < 0 || toIdx < 0) return;
          var movedP = fromComp.projects.splice(fromIdx, 1)[0];
          fromComp.projects.splice(toIdx, 0, movedP);
          this.tree = this.tree.slice();
          restoreScroll();
          this.showToast2('순서 변경됨');

          // 백그라운드 API (병렬)
          var newIds = fromComp.projects.map(function(p) { return p.id; });
          Promise.all(newIds.map(function(id, i) {
            return fetch('/api/contacts/projects/' + id, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ order: i })
            });
          ...
          // ── 다른 업체로 현장 이동 — optimistic ──
          var movedProj = fromComp.projects.find(function(p) { return p.id === dragId; });
          fromComp.projects = fromComp.projects.filter(function(p) { return p.id !== dragId; });
          movedProj.companyId = toComp.id;
          toComp.projects.push(movedProj);
          this.tree = this.tree.slice();
    ...
    // ── 현장을 다른 업체로 이동 ──
    openMoveProject(proj, fromCompanyId) {
      this.projectMoveModal = { open: true, proj: proj, fromCompanyId: fromCompanyId, targetCompanyId: '' };
    },
    async saveMoveProject() {
      var m = this.projectMoveModal;
      if (!m.proj || !m.targetCompanyId) { this.showToast2('업체를 선택하세요'); return; }
      if (m.targetCompanyId === m.fromCompanyId) { this.projectMoveModal.open = false; return; }
      await fetch('/api/contacts/projects/' + m.proj.id, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ companyId: m.targetCompanyId })
      });
      this.projectMoveModal.open = false;
      await this.loadTree();
      this.showToast2(m.proj.name + ' → 업체 이동 완료');
    },

    // ── 단일 연락처 현장 이동 ──
    openContactMove(contact) {
      // 현재 소속 업체/현장으로 초기화
      this.contactMoveModal = {
        open: true,
        contact: contact,
        targetCompanyId: contact.companyId || '',
        targetProjectId: contact.projectId || ''
      };
    },
    get contactMoveProjectOptions() {
      if (!this.contactMoveModal.targetCompanyId) return [];
      var cid = this.contactMoveModal.targetCompanyId;
      var comp = this.tree.find(function(c) { return c.id === cid; });
      return comp ? comp.projects : [];
    },
    async saveContactMove() {
      var m = this.contactMoveModal;
      var c = m.contact;
      if (!c || !m.targetCompanyId) { this.showToast2('업체를 선택하세요'); return; }

      var compName = '';
      var projName = '';
      var self = this;
      this.tree.forEach(function(comp) {
        if (comp.id === m.targetCompanyId) {
          compName = comp.name;
          comp.projects.forEach(function(p) {
            if (p.id === m.targetProjectId) projName = p.name;
          });
        }
      });

      var upd = { companyId: m.targetCompanyId, projectId: m.targetProjectId || '', company: compName, note: projName };
      await fetch('/api/contacts/' + c.id, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(upd)
      });
      this.contactMoveModal.open = false;
      await this.loadTree();
      this.showToast2((c.name || '연락처') + ' → ' + (projName || compName) + ' 이동 완료');
    },
    ...
    sortedContacts(contacts) {
      if (!this.sortField) return contacts;
      var field = this.sortField;
      var dir = this.sortDir;
      // 직책 계층 순서 (낮은 숫자 = 높은 직급)
      var positionRank = [
        ['대표이사', 1],
        ['고문', 2],
        ['재무이사', 3], ['이사', 3],
      ...
      function getPosRank(pos) {
        if (!pos) return 999;
        // 긴 키부터 매칭해서 '부사장'이 '사장'으로 잘못 매칭되는 것 방지
        for (var ki = 0; ki < positionRank.length; ki++) {
          if (pos.indexOf(positionRank[ki][0]) >= 0) return positionRank[ki][1];
        }
        return 999;
      }
      return contacts.slice().sort(function(a, b) {
        if (field === 'position') {
          var ar = getPosRank(a.position);
          var br = getPosRank(b.position);
