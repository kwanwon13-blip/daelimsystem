...
- **안전 강도: "중간"(사용자 선택).** 사내망+신뢰 직원 환경 기준. 깨끗한 업무 사실은 즉시 활성·주입(마법 유지), **위험 신호(개인정보·비밀·명령형/포이즈닝·부정교정)만 자동 보류→관리자 검토**. 상세 §12.2.
...
### 12.2 ★ 위험 신호만 보류하는 게이트 (안전강도 = "중간", 사용자 선택)
우리는 사내망 + 신뢰 직원 환경이라 Odysseus(공개 서비스, 낯선 공격자 가정)보다 위협이 낮다. 그래서 **"전부 pending"이 아니라 "위험한 것만 pending"**으로 calibration.

- **기본: 추출된 깨끗한 업무 사실은 `status='active'`로 즉시 저장·주입** (마법 유지, 관리 품 최소).
- **위험 신호가 걸리면 `status='pending'`(주입 안 함) → 관리자 검토 큐로.** 위험 신호 =
  1. 개인정보/비밀 패턴(§12.3) — 주민번호·계좌·급여+인명·연락처·API키 등
  2. 명령형/탈옥 문구(§12.1) — "~해라/항상 ~하라/무시하라/instead/system" 등(포이즈닝 의심)
  3. 인물·인사·연락처 카테고리(화이트리스트 밖)
  4. 부정·교정 모호(§12.5) — "안 함/별도 아님/이제 ~안 씀" 등
- **관리자는 "회사 기억" 화면에서** active 항목을 언제든 삭제/수정/고정하고(=솎음), pending 큐를 승인/기각.
- 스키마 `status`: `pending | active | archived`. (참고: 더 엄격히 가려면 "active 승격 전 hit_count>=2" 옵션을 켜는 스위치만 남겨두되 기본 off.)
