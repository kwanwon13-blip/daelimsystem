...
async function generateQuotePdf(quoteData, namecardImgPath) {
  ...
  // quote-print.html 읽기
  const templatePath = path.join(__dirname, 'public', 'quote-print.html');
  ...
  // 이미지 경로를 절대경로로 변환
  const dataDir = path.join(__dirname, 'data').replace(/\\/g, '/');
...
// 회원가입 잠금 토글 (관리자 전용)
app.post('/api/settings/registration-lock', requireAuth, requireAdmin, (req, res) => {
  const settingsPath = path.join(__dirname, 'data', 'settings.json');
  let settings = {};
  if (fs.existsSync(settingsPath)) settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  settings.registrationClosed = !settings.registrationClosed;
  fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2), 'utf8');
  const state = settings.registrationClosed ? '잠금' : '열림';
  auditLog(req.user.userId, `회원가입 ${state}`, '설정');
  res.json({ ok: true, registrationClosed: settings.registrationClosed, message: `회원가입이 ${state} 상태로 변경되었습니다.` });
});
...
app.post('/api/auth/register', (req, res) => {
  ...
  let settings = {};
  if (fs.existsSync(settingsPath)) settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  if (settings.registrationClosed) return res.status(403).json({ error: '현재 신규 가입이 마감되었습니다. 관리자에게 문의하세요.' });
...
app.post('/api/admin/users/:id/profile', requireAdmin, (req, res) => {
  ...
  if (req.body.capsName !== undefined) user.capsName = req.body.capsName; // CAPS 등록 이름 (출퇴근 매핑용)
  db.saveUsers(uData);
  res.json({ ok: true });
});
...
// - 관리자: 전체 열람 가능
// - 팀장(부서 리더): 본인 작성 + 본인 승인 대상 + 같은 부서원이 작성한 문서
// - 일반 직원: 본인 작성 + 본인이 승인자로 지정된 문서만
...
// 결재 문서 목록 조회 (내가 작성 / 내가 승인할 / 접근 가능한 전체)
...
app.post('/api/approvals/:id/process', requireAuth, (req, res) => {
  ...
  if (action === 'approved' && doc.type === 'leave' && doc.formData) {
    try {
      ...
      // 이렇게 해야 출퇴근 기록부 탭에서 해당 날짜에 연차가 표시됨
      try {
        const attData = db.출퇴근관리.load();
        if (!attData.attendanceNotes) attData.attendanceNotes = {};
        // 직원의 CAPS employeeId 찾기 (이름 매칭)
        const attEmpId = doc.authorName; // CAPS는 이름을 키로 사용
        // leaveType 코드 매핑 (attendanceNotes 형식에 맞게)
        ...
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
          ...
          // 이미 수동 노트가 있으면 덮어쓰지 않음 (관리자가 직접 설정한 것 우선)
...
app.delete('/api/approvals/:id', requireAuth, (req, res) => {
  ...
  // 소프트 삭제: status를 'deleted'로 마킹, 원본 status 보존
  ...
  // 결재가 승인 상태였다면, 해당 결재로 생성된 연차 기록도 함께 삭제
...
// 결재 문서 복구 (admin 전용)
app.post('/api/approvals/:id/restore', requireAuth, (req, res) => {
...
// 파일 직접 다운로드 (외부 접속 대응)
app.get('/api/design/file', requireAuth, (req, res) => {
  const filePath = req.query.path;
  if (!filePath) return res.status(400).send('path required');
  const resolved = path.resolve(filePath);
...
app.get('/api/attendance/summary', requireAuth, async (req, res) => {
  ...
  // ── 최종 안전장치: admin 역할 사용자 응답에서 완전 제거 ──
...
// ─────────────────────────────────────────────────────────
app.get('/api/leave/compare', requireAuth, (req, res) => {
  try {
...
// 서버 시작 시 데이터 마이그레이션
try { migrateContactsData(); } catch(e) { console.error('⚠️ 연락처 마이그레이션 실패 (서버는 계속 실행):', e.message); }
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   자동동기화: ${AUTO_SYNC_TIMES.map(t => `${String(t.h).padStart(2,'0')}:${String(t.m).padStart(2,'0')}`).join(', ')} (평일)\n`);
});
