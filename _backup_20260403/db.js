...
const jsonStores = {
  ...
  '출퇴근관리': {
    ...
    exemptDepts: []         // 근태면제 부서 (지각/조퇴/출근미기록 체크 안함)
  },
  '감사로그': {
    logs: []
  },
...
function loadStore(storeName) {
  ...
  try {
    ...
    if (fs.existsSync(backupPath)) {
      console.warn(`⚠️ ${storeName}.json 손상 → 백업 복구`);
      data = JSON.parse(fs.readFileSync(backupPath, 'utf8').replace(/\0/g, ''));
      fs.copyFileSync(backupPath, filePath);
    } else {
      throw new Error(`${storeName}.json 손상 (백업 없음): ${e.message}`);
    }
  }
...
function saveStore(storeName, data) {
  ...
  if (storeName !== '감사로그' && fs.existsSync(filePath)) {
    try {
