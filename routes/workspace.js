...
4. 한국어로 작성. 대림에스엠은 단가표/견적/시안/출퇴근/결재 업무를 하는 안전 종합 회사.
...
// 카톡 발주 메시지 → 발주 표(품목·규격·수량·현장·납기·상태) + '확인 필요' 체크리스트
const ORDER_SYSTEM_PROMPT = `당신은 대림에스엠 ERP 워크스페이스의 '카톡 발주 정리' 도우미입니다.
직원이 카카오톡으로 받은 발주/주문 메시지(다듬어지지 않은 대화체)를 받아, 발주 항목을 표로 정리한 JSON 블록을 생성하세요.

== 규칙 ==
1. 출력은 **반드시 순수 JSON** 만. 설명/머릿말/\`\`\`코드블록\`\`\` 절대 금지.
2. JSON 스키마: { "blocks": [ {type, data}, ... ] }
3. 블록 순서(고정):
   1) header level 2 — "📦 발주 정리" (거래처/현장명을 알면 뒤에 덧붙임)
   2) dataTable — 발주 품목 표. columns 는 정확히 이 6개:
      [{"id":"c1","name":"품목","type":"text"},{"id":"c2","name":"규격","type":"text"},{"id":"c3","name":"수량","type":"number"},{"id":"c4","name":"현장","type":"text"},{"id":"c5","name":"납기","type":"date"},{"id":"c6","name":"상태","type":"select","options":["대기","진행","완료"]}]
      rows: 메시지에서 읽어낸 품목을 1줄씩. 상태(c6)는 모두 "대기".
   3) checklist — "확인 필요". 빠졌거나 불명확한 정보(수량/규격/납기/현장 미기재 등)를 체크항목으로. 없으면 이 블록은 생략.
4. **모르는 값은 빈 문자열("")로 두고 절대 지어내지 마세요.** 납기(c5)는 확실하면 YYYY-MM-DD, 아니면 "".
5. rows 의 key 는 columns id(c1~c6)와 정확히 일치. 한국어로.
6. 발주 내용이 전혀 안 보이면 header + paragraph("발주 내용을 인식하지 못했어요. 메시지를 확인해주세요.") 두 블록만 출력.

== 예시 ==
입력: "내일까지 강남현장 30T 투명유리 10장이랑 9T 5장 부탁해요"
출력:
{"blocks":[
{"type":"header","data":{"text":"📦 발주 정리 — 강남현장","level":2}},
{"type":"dataTable","data":{"columns":[{"id":"c1","name":"품목","type":"text"},{"id":"c2","name":"규격","type":"text"},{"id":"c3","name":"수량","type":"number"},{"id":"c4","name":"현장","type":"text"},{"id":"c5","name":"납기","type":"date"},{"id":"c6","name":"상태","type":"select","options":["대기","진행","완료"]}],"rows":[{"c1":"투명유리","c2":"30T","c3":10,"c4":"강남현장","c5":"","c6":"대기"},{"c1":"투명유리","c2":"9T","c3":5,"c4":"강남현장","c5":"","c6":"대기"}]}},
{"type":"checklist","data":{"items":[{"text":"납기 '내일'의 정확한 날짜 확인","checked":false}]}}
]}

이제 아래 발주 메시지를 위 형식의 JSON 으로만 정리하세요. 다른 텍스트 금지.`;
...
async function callClaudeForBlocks(userPrompt, systemOverride) {
  const content = await claudeClient.callClaude({
    system: systemOverride || WS_AI_SYSTEM_PROMPT,
    user: userPrompt,
    maxTokens: 4096,
  });
...
router.post('/ai-generate', async (req, res) => {
  try {
    ...
    const isOrder = (req.body && req.body.mode) === 'order';
    if (!prompt) return res.status(400).json({ error: '프롬프트가 비어있습니다' });
    const maxLen = isOrder ? 3000 : 1000;
    if (prompt.length > maxLen) return res.status(400).json({ error: '입력은 ' + maxLen + '자 이하로 작성해주세요' });

    const rawBlocks = await callClaudeForBlocks(prompt, isOrder ? ORDER_SYSTEM_PROMPT : null);
