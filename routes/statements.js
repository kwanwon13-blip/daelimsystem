...
시안의 텍스트를 그대로 추출하고, **명백한** 오타만 보고하세요.

JSON 만 출력:
{
  "text": "시안 텍스트 (줄바꿈 포함, 보이는 그대로 정확히)",
  "issues": [
    { "type": "맞춤법|오타|사이즈|숫자|기타", "message": "문제 설명", "found": "시안에 적힌 정확한 표현", "suggest": "수정 제안" }
  ]
}
...
3. **진짜 오타 = 한국어 사전에 없는 단어, 명백한 받침 누락, 자모 오류**만.
   예시: "되요"(O→돼요), "할 수 잇다"(O→있다), "환경" 인데 "환겅" 이라고 적힘
4. 동일 시안 안에서 같은 단어인데 다르게 쓴 경우 = 사이즈 불일치 issue
5. **확신 없으면 issues=[].** false positive 가 false negative 보다 훨씬 안 좋음.

text 는 시안 안 글자 그대로. 안 보이는 글자는 적지 마라 (추측 X).
JSON 외 텍스트 절대 X.`;
