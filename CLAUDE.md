...
## Codex/Claude 자동 배포 규칙 — push/pull/restart (새 채팅 필수)

사용자가 "푸시풀", "배포", "서버 반영"을 요청하면 아래 절차를 따른다. 이 섹션은 이전에 적힌 로컬 `.env` 기반 pull 예시나 `git-setup.bat` / `git-pull-server.bat` 수동 절차보다 우선한다.
...
> ## ⚠️ 저장 데이터 기본 규칙 — **무조건 SQLite (사장님 지시 2026-06-23)**
> **새로 저장하는 데이터는 JSON 말고 무조건 SQLite로 저장한다. 신규 JSON 스토어 생성 금지.**
> - 새 영속 데이터 → `db-sqlite.js`에 테이블 추가(`db.sql.*`). JSON 파일로 새로 저장하지 말 것.
> - 이유: JSON은 매 요청 **전체 재파싱** + 매 저장 **전체 재기록**이라 데이터 늘수록 느려짐. SQLite는 색인·부분 읽기/쓰기로 확장.
> - 기존 JSON 스토어(`workflow.json` 등)는 SQLite로 **이관 중**: 이력(events) 완료(1e75a36), 작업·파일·발주 진행. JSON은 이관 중 **백업/폴백으로만** 잔존(절대 삭제 X).
> - ★ 저장 방식 같은 핵심 결정은 **만들기 전에 먼저 확정**하고 시작한다(다 만든 뒤 방식이 틀리면 안 됨).
...
## gstack 브라우저 디버깅 (Claude in Chrome 사용 시)
```
